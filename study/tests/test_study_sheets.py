...
two things that matter most: no leakage of stimulus_id/kind/arm to the
participant, and that a filled-in sheet round-trips into exactly the
schema run_study.py itself writes.
"""
...
def _fill_sheet(path, responses):
    from openpyxl import load_workbook

    wb = load_workbook(path)
    ws = wb["Responses"]
    for i, (label, confidence, warn) in enumerate(responses, start=2):
...
def _valid_responses():
    return [(RESPONSE_LABELS[i % 5], (i % 5) + 1, "Yes" if i % 2 else "No") for i in range(len(STIMULI))]
...
def test_control_arm_never_shows_technique_cards():
    ...
    ws = wb["Responses"]
    for row in ws.iter_rows(min_row=2, max_row=1 + len(STIMULI), values_only=True):
...
def test_round_trip_matches_run_study_schema(tmp_path, monkeypatch):
    ...
    for line, stimulus in zip(lines, STIMULI):
        ...
        assert line["response"] in RESPONSE_CHOICES
